...
from pyparsec.Parsec import State, initial_pos, SourcePos
from pyparsec.Prim import tokens, run_parser, token
...
def test_list_of_objects():
    ...
    parser = match_kind("ID") >> (lambda x: 
             match_kind("OP") >> (lambda y: 
             match_kind("NUM").map(lambda z: (x, y, z))))
...
def test_parsing_tokens_into_ast_object():
    ...
    # Syntax: x = 100
    assignment_parser = identifier.bind(lambda name: 
                        equals >> 
                        number.map(lambda val: AssignmentNode(name, val)))
